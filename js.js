...
/**
 * Объект подсчитывает очки пользователя
...
const config = {
  ...
  /**
   * Возвращаем края игрового поля для snake.
   * @return {array} Массив со значения ми границ игрового поля.
   */
  getRowAndColsCount() {
    return [this.settings.colsCount, this.settings.rowsCount];
  },
...
const snake = {
  ...
  init(startBody, direction) {
    ...
    // Устанавливаем края игрового поля.
    this.setMaxXAndMAxY();
  },

  /**
   * Задаем края игрового поля для сквозного перемещения змейки.
   */
  setMaxXAndMAxY() {
    this.maxX = config.getRowAndColsCount()[0] - 1;
    this.maxY = config.getRowAndColsCount()[1] - 1;
  },
  ...
  getNextStepHeadPoint() {
    ...
    // Возвращаем точку, где окажется голова змейки в зависимости от направления, проверяем на граници поля для сквозного передвижения.
 ...
 * @property {int} score Счетчик очков пользователя
 */
const game = {
  config,
  map,
  ...
  init(userSettings) {
    ...
    // устанавливаем счетчик в начальное положение.
    this.score.init();
  ...
  reset() {
    ...
    // Ставим счетчик очков пользователя на 0.
    this.score.drop();
